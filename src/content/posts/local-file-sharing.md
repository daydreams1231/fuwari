...
# Samba / KSMBD 区别
SMB是Windows下的文件分享协议, 也是著名零日漏洞永恒之蓝的核心原理; 而Samba是对SMB协议的Linux实现 <br>
KSMBD类似Samba, 不过其工作在内核空间, 自Linux 5.15后并入主线, 相较于另一个Samba性能更优, 但功能更少, 兼容性也差一点 <br>

# KSMBD配置
## 安装
默认情况下ksmbd应该已编译到内核中, 只需要安装一个用户空间的控制工具即可
```shell
sudo apt install ksmbd-tools
```

## 配置共享用户
ksmbd 的用户不需要在linux系统中存在，这与SMB不同. 但实际操作中不建议这样做, 尽量添加一个主机中存在的用户. 如果只有root用户, 要么使用root用户, 要么改文件权限o=777
```shell
...
:::
> 一般推荐将文件夹的所有者改为先前设置的登录用户, 再chmod -R 770 共享
...
> 各Linux发行版的ksmbd-tools版本汇总: https://pkgs.org/download/ksmbd-tools
...
```
## 配置
/etc/samba/smb.conf: samba主配置文件, 和ksmbd.conf的类似, 但某些选项有差异, 具体自己看man手册 <br>
```
# global
unix charset = UTF-8
dos charset = GBK
...
```
如果是OpenWRT, 需要先安装nfs-utils 和 kmod-fs-nfs包  <br>

如果挂载提示 No Such Device, 请检查: <br>
lsmod | grep nfs  <br>
看是否有nfs这个模块 <br>
如果是nfsd, 这是NFS服务器上用的, 在客户端没有用
