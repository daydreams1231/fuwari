...
# 前言
随着国内各大运营商对PCDN严打的力度不断加强(其实是针对所有高上传的"负价值用户"), 个人难以申请公网IPv4, 以及个人用户很难获得光猫管理权限等种种原因, 国内BT环境可以说越来越烂, 另外还有部分主动跑PCDN的脑残, 会选择使用吸血BT客户端来刷下行流量, 如果你愿意面对这些困难, 本文即为解决这些问题而生.

:::tip[阅前提醒]
本文可能不适合PT用户; 也不对任何后果负责
:::

## 软件选择
Windows下我一般用`Bitcomet`或者`qBittorrent Enhanced Edition` (下文称为QBEE), Bitcomet的长效种子有一定用, 但效果不多, 一般只有热门种子才有长效种, 总之还是不如更好的网络环境来的实在 <br>
Linux下只用QBEE, 没别的, 就为了跨平台webui <br>
另外, 使用`PeerBanHelper` + BTN网络的方式来反吸血 <br>

## QBEE调优
`连接` 下的上传窗口代表对于单个任务可连接的用户数量, 建议根据自己的上传带宽来调: (以下的上行带宽指你愿意分给BT的上行带宽, 而不是宽带总上行)
  - 上行带宽 <=5Mbps: 4个
  - 10 - 30Mbps: 8-10个
基本上是 1Mbps对应一个上传窗口 <br>

`每Torrent最大连接数`代表单个任务可连接的用户数量, 这个不用太大, 我设的64, 再多没意义, 毕竟正常下载过程中你的下载速度主要由一小部分用户提供, 别的用户的流量可以忽略. <br>
这个值过大可能会导致你的网络变卡, 因为运营商家宽的连接数不会太多(比如我这移动只有2000左右的连接数) <br>

`Peer连接协议`保持默认的`TCP+uTP`即可, 除非你的环境存在严重的UDP限速
...
`通过代理查找主机名`: 控制DNS解析是在本地还是代理上完成(看源码貌似是socks5h和socks5的区别, 所以为了防止DNS污染, 建议打开) (DNS解析操作有: RSS, Tracker服务器等)<br>
`对 BitTorrent 目的使用代理`: qBittorrent 上报给 tracker server 的 IP 是否是代理服务的IP <br>
`使用代理服务器进行用户连接`: 是否使用代理来连接Peer, 用于隐藏自己的IP, 开启此选项后会向Tracker服务器上报本地监听端口为1(这个我在源码里暂时没找到, 但是通过fiddler抓包发现确实是这样), 导致其他用户无法连接, 也就无法做种<br>
...
# 网络优化
众所周知, 国内大部分地区的家庭用户是没有公共IPv4的, 并且还会套好几层NAT, 就算你有公网IP, 运营商也可能会对你的连接数进行限制, 或者UDP QoS限速

由于国内用到基本是光猫路由一体的机器, 这类机器没有动态超级密码的情况下你改不了什么东西, 而大部分地区又要不到公网IPv4, 这种情况下, 网上说的什么在路由器那配置DMZ Upnp 防火墙什么的我个人感觉没多大意义, 但我还是会把这些写出来, 多一条选择. <br>
### DMZ
你并不需要了解DMZ的概念, 你只需要知道, DMZ一般只能设置一个主机, 设置后, 路由器对于收到的数据包, 如果其不在NAT表中, 会把数据包转发给DMZ主机 <br>
当然, 前提是你的路由器得有公网IP, 这样你才能收到外部发来的数据包 <br>
> 一般设置了DMZ的主机, 路由器防火墙不对其生效 <br>

> 部分路由器的DMZ可能只对IPv4生效

### UPNP / 虚拟服务器
两者功能其实类似, 前者可以通过软件自动配置, 而后者必须手动在路由器里设置 <br>
一般其会让你设置一个外部端口和一个内部端口+内部IP, 这里的外部端口其实就是路由器wan口的端口, 在wan区域收到发往特定端口的数据包时, 路由器才会将其转发到目标内网主机 <br>

> 和DMZ一样, 也是要你能收到数据包, 这里最常用的是IPv6虚拟服务器, 毕竟绝大部分用户都是有IPv6的 <br>

> 此项优先级一般大于防火墙 <br>

## 另辟蹊径
假设你刚好有一台VPS, (没有也没关系, 阿里云99就能买个续费同价的3M不限流量服务器, 还是很划算的), 可以使用中转的方式玩BT <br>

Bittorrent下载需要和Peer建立连接, 这分为两种: `你连接到Peer` & `Peer连接到你` <br>

前者可以通过BT软件内置的代理, 让我们通过代理连接到Peer, 避免因GFW等网络原因导致的无法连接 <br>
> 使用代理时, 在Tracker看来, 你的出口IP是VPS的IP, 由于端口只能通过tracker报文获取, 你的监听端口还是本地的端口

这种方法需要在VPS上部署一个代理服务, 如果你的VPS在国内, 就用简单的socks5代理协议就行, 但如果VPS在国外, 就需要使用更为高级的代理软件如Xray或者Hysteria2, 原因无他, 抗GFW. (如果你比较注重隐私, 而VPS在国内, 也可以使用这些高级代理软件) <br>

> 如果使用国内VPS, 更推荐使用wireguard VPN, 再让BT软件绑定这个接口, 这个的前提是vpn能路由全部流量

:::warn[警惕版权投诉]
VPS选择要慎重, 由于国外对版权管理比较严格, 在VPS上使用BT可能会导致你的VPS被DMCA投诉
:::
举个例子: <br>
本地BT通过socks代理连接到 运行在本地或局域网的代理转发软件(推荐Xray), Xray将BT流量封装成更抗GFW的流量, 送到你的VPS上, VPS上的Xray还原成BT流量, 完成访问 <br>
...
> 由于源IP信息丢失, 这两种方法显然也无法对抗吸血客户端 <br>
...
:::
之后下载种子时, 如果QBEE里能看到用户标签里有`I`, 或者Bitcomet里`发起`为`远程`, 即代表成功 <br>

这个VPN隧道中转的方法来自: [Ref](https://www.bilibili.com/opus/1104998878057332745) <br>
...
如下配置使用gost作为创建隧道的软件, 其使用socks5作为传输介质, 携带数据数据流, 再通过Xray进行传输, 但为了方便验证, `server.json`中的入站部分和`client.json`中的出站部分仅仅使用了vless+TCP传输, 实际使用时记得修改这两部分. <br>
